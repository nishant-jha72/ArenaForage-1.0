...
import { Quote, ChevronLeft, ChevronRight, Play, ArrowRight } from 'lucide-react';

// ==========================================
// DOTENV CONFIGURATION HELPERS
// ==========================================
//
// Create a .env file in your project root:
//
// --- For Vite ---
// VITE_YOUTUBE_VIDEO_ID=your_video_id_here
// VITE_VIDEO_PLACEHOLDER_IMAGE=https://your-cdn.com/image.jpg
//
// --- For Create React App ---
// REACT_APP_YOUTUBE_VIDEO_ID=your_video_id_here
// REACT_APP_VIDEO_PLACEHOLDER_IMAGE=/homepage/local-image.jpg
//
// Priority: Component Props > .env Variables > Default Fallbacks

/**
 * Reads environment variables supporting both Vite (VITE_*) and CRA (REACT_APP_*) prefixes.
 */
...
/**
 * Resolves image source:
 * - If the value is a full URL (http/https), use it directly.
 * - Otherwise, treat it as a local/system image path.
 */
...
const ENV_YOUTUBE_VIDEO_ID = getEnvVar('YOUTUBE_VIDEO_ID', 'YOUR_YOUTUBE_VIDEO_ID');
const ENV_PLACEHOLDER_IMAGE = resolveImageSource(
  'VIDEO_PLACEHOLDER_IMAGE',
  '/homepage/video-placeholder.jpg'
);
...
export interface HomepageAdProps {
  /** Overrides .env VITE_YOUTUBE_VIDEO_ID / REACT_APP_YOUTUBE_VIDEO_ID */
  youtubeVideoId?: string;
  /** Overrides .env VITE_VIDEO_PLACEHOLDER_IMAGE / REACT_APP_VIDEO_PLACEHOLDER_IMAGE */
  placeholderImage?: string;
  /** Array of community reviews */
...
const TestimonialSlide: React.FC<{
  ...
  return (
    ...
    >
      <div className="mb-4">
        <Quote className="w-8 h-8 text-amber-500/30" strokeWidth={1.5} aria-hidden="true" />
      </div>
      ...
      <footer className="flex items-center gap-3">
        ...
        <div>
          ...
          {review.role && <p className="text-xs sm:text-sm text-gray-400">{review.role}</p>}
        </div>
      </footer>
    </div>
  );
...
/**
 * Left Panel: Custom image placeholder that swaps to a YouTube iframe on click.
 */
...
export const HomepageAd: React.FC<HomepageAdProps> = ({
  ...
  const finalPlaceholderImage = placeholderImage || ENV_PLACEHOLDER_IMAGE;
  ...
  const goTo = useCallback(
    (index: number, dir: 'next' | 'prev' = 'next') => {
      if (isTransitioning || index === activeIndex || reviews.length <= 1) return;
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 xl:gap-16 items-center w-full">
          ...
          <div className="order-1 w-full">
            <VideoEmbed videoId={finalVideoId} placeholderImage={finalPlaceholderImage} />
          </div>
